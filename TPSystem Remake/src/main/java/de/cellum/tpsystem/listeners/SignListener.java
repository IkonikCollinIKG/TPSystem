...
public class SignListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (block.getType() == Material.OAK_SIGN) {
                ...
                if (sign.getLine(0).equalsIgnoreCase("Westonia") && sign.getLine(1).equalsIgnoreCase("Reise") && sign.getLine(2).equalsIgnoreCase("System")) {
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        ...
        if (block.getType() == Material.OAK_SIGN) {
            Sign sign = (Sign) block.getState();
            if (sign.getLine(0).equalsIgnoreCase("Westonia") &&
                    sign.getLine(1).equalsIgnoreCase("Reise") &&
                    sign.getLine(2).equalsIgnoreCase("System")) {
